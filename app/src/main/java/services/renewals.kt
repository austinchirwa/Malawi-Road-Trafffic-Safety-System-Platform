...
@Composable
fun RenewalsScreen(onBackClick: () -> Unit, onNavigateToPayment: () -> Unit) {
                ...
                Button(
                    onClick = {
                        ...
                        if (fullName.isNotBlank() && idNumber.isNotBlank() && licenseNumber.isNotBlank() && licenseCode.isNotBlank() && vehicleRegNumber.isNotBlank() && trnNumber.isNotBlank()) {
                            Toast.makeText(context, "Application submitted! Proceeding to payment...", Toast.LENGTH_SHORT).show()
                            onNavigateToPayment()
                        } else {
